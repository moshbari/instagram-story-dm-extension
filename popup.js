document.addEventListener('DOMContentLoaded', () => {
    ...
    saveBtn.addEventListener('click', () => {
        // Handle Importing Today's Skip List (users to skip for today only)
        ...
        if (importBox && importBox.trim().startsWith('[')) {
            try {
                ...
                chrome.storage.local.get(['messaged_today'], (res) => {
                    let messaged_today = res.messaged_today || { date: todayStr, users: [] };
                    if (messaged_today.date !== todayStr) {
                        messaged_today = { date: todayStr, users: [] };
                    }
                    messaged_today.users = [...new Set([...messaged_today.users, ...importedArray])];
                    chrome.storage.local.set({ messaged_today });
                    document.getElementById('importMemory').value = "";
                    saveBtn.textContent = "Imported for Today & Saved!";
                });
            } catch (e) {
                console.error("Failed to parse imported list.", e);
            }
        }
